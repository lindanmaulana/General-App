...
export const FormatFileConfig = () => {
    ...
    const isActive = (format: string): string => format === formatFile ? "dark:border-white border-2 border-gnrDark" : "dark:border-white/20 dark:hover:border-white/80 hover:border-gnrDark"

    return (
        <div className="space-y-2">
            <h4 className="dark:text-gnrWhite text-lg font-medium">Format File</h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Card onClick={() => hanldeSetConfig({"format": "excel"})} className={`${isActive("excel")} dark:bg-black cursor-pointer`}>
                ...
                <Card onClick={() => hanldeSetConfig({"format": "csv"})} className={`${isActive("csv")} dark:bg-black cursor-pointer`}>
                    <CardContent className="flex flex-col items-center justify-center">
                ...
                <Card onClick={() => hanldeSetConfig({"format": "pdf"})} className={`${isActive("pdf")} dark:bg-black cursor-pointer`}>
                    <CardContent className="flex flex-col items-center justify-center">
                        <FileText className="dark:text-white mb-1" />
                ...
                <Card onClick={() => hanldeSetConfig({"format": "json"})} className={`${isActive("json")} dark:bg-black cursor-pointer`}>
                    <CardContent className="flex flex-col items-center justify-center">
