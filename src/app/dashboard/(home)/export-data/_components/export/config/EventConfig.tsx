                ...
                {data?.map((event: events) => {
                    ...
                    <BaseCard key={event.id} style='p-2 rounded-md'>
                        <BaseCardContent style='flex gap-3 p-2'>
                            <Checkbox onCheckedChange={(check) => handleSetConfig(check ? {events: [...events, event.code]} : {events: events.filter(e => e !== event.code)})} className='size-5 active:bg-gnrPrimary/20 cursor-pointer' checked={events.includes(event.code)} />
