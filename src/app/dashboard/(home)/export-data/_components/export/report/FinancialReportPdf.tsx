...
export const FinancialReportPdf = ({ incomes, expenses, totalBalance }: initialData) => {
    ...
    const totalExpensesNumber: number =
        expenses.length > 0 ? expenses.reduce((acc, call) => (acc += call.amount), 0) : 0;
                        ...
                        <View style={styles.table}>
                            ...
                            {data.incomesData.map((incomes) => (
                                <View key={incomes.No} style={styles.tableRow}>
                                    <Text style={[styles.tableCol, { width: "6%" }, styles.number]}>{incomes.No}</Text>
                                    <Text style={[styles.tableCol, { width: "20%" }]}>{incomes.Tanggal}</Text>

                                    <Text style={[styles.tableCol, { width: "30%" }]}>{incomes.Sumber_Pendapatan}</Text>

                                    <Text style={[styles.tableCol, { width: "22%" }]}>{incomes.Tipe_Akun}</Text>

                                    <Text style={[styles.tableCol, { width: "22%" }]}>{incomes.Jumlah_Rupiah}</Text>
                                </View>
                            ))}
