        ...
        const column:ColumnDef<events>[] = [
            ...
            },
            {
                ...
                cell: ({row}) => {
                    ...
                    return <Badge className={cn(event.status === "SCHEDULED" ? "text-gnrPrimary bg-gnrPrimary/20 hover:bg-gnrPrimary/50" : event.status === "RUNNING" ? "text-gnrOrange bg-gnrOrange/20 hover:bg-gnrOrange/50" : event.status === "COMPLETED" ? "text-gnrGreen bg-gnrGreen/20 hover:bg-gnrGreen/50" : event.status === "CANCELLED" ? "text-gnrRed bg-gnrRed/20 hover:bg-gnrRed/50" : "text-gnrRed bg-gnrRed/20")}>{event.status}</Badge>
                }
            },
            {
                accessorKey: "budget",
